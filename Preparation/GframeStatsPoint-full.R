...
max.mean.new <- function(y, n.guess=floor(sqrt(N))){
  message("Please use max.mean instead.")
  #always choose m=n
  m <- n.guess
  y <- y[1:(m*n.guess)]
  y.mat <- matrix(y, ncol = n.guess, byrow = TRUE)
...
#we use the duality between lower and upper cdf of G-normal 
#to get lower version of cdf, pdf, quantile from the upper version

pGnorm <- function(z, sdl=.5, sdr=1, upper = TRUE){
  upcdf <- function(z){
    C1 <- 2*sdr/(sdl+sdr)
    C2 <- 2*sdl/(sdl+sdr)
...
qGnorm <- function(p, sdl=.5, sdr=1, upper = TRUE){
  upquantile <- function(p){
    K <- sdr/(sdl+sdr)
    if(p<=K){
      sdr*qnorm(p/(2*K))
    } else {
...
dGnorm <- function(z, sdl=.5, sdr=1, upper = TRUE){
  uppdf <- function(z){
...
pSemiGnorm <- function(z, sdl=.5, sdr=1, upper = TRUE){
  upcdf <- function(z){
...
qSemiGnorm <- function(p, sdl=.5, sdr=1, upper = TRUE){
  upquantile <- function(p){
    if(p<=1/2){
      sdr*qnorm(p)
    } else {
...
dSemiGnorm <- function(z, sdl=.5, sdr=1, upper = TRUE){
  uppdf <- function(z){
