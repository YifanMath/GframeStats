...
#min(Z_i)
#upper p-values
uppvalue.minmean <- function(c, m, sdl=0.5, sdr=1, pf = pGnorm){
  upprob.less.c <- 1 - (1 - pf(c, sdl = sdl, sdr = sdr))^m
  upprob.less.c
}
#also check it for semi-G-normal 
...
uppvalue.comb <- function(a,b,m, sdl=0.5, sdr=1, 
  ...
  1-(1-pf(-b,sdl=sdl,sdr=sdr)-pf(a,sdl=sdl,sdr=sdr))^m
}
...
#it can also be used to check covariance uncertainty 
#consider the sequence X[t]*X[t-1]

get.smooth.val <- function(y,x){
  #extract value from geom_smooth
  mod <- mgcv::gam(formula = y ~ s(x, bs = "cs"), method = "REML")
  mod$fitted.values
}
...
testUncertainty <- function(dat.seq, ref.seq=NULL,
                            ...
                            M.rep=5e2,
                            hist.bin=35, 
                            return.ind=TRUE, all.plot = TRUE, plot.re = TRUE, 
                            sig.level = .05, 
                            sd.vec.true = NULL){
  
  #test.plot = TRUE: includes the four visualizations of extreme-mean curves
  ...
  if (test.new.re){
    #whether to print one test result for a chosen group size
    ...
    p.maxmean <- uppvalue.maxmean(sqrt(n.check)*max(dat2.meanseq.std), 
                                  sdl = sdl.est, sdr = sdr.est, m=m.check, 
                                  pf = pf.asym)
    p.minmean <- uppvalue.minmean(sqrt(n.check)*min(dat2.meanseq.std), 
                                  sdl = sdl.est, sdr = sdr.est, m=m.check, 
                                  pf = pf.asym)
  ...
  if (test.new.plot){
    ...
    if(is.null(n.check.seq)){
      if(n.consistent.ind){
        n.check.seq <- n.seq
        dat2.minmax.mat.check <- dat2.minmax.mat - mean(dat2.seq)
      } else {
        #n.check.seq <- seq(1e2, floor(N/10), 20)
        dat2.minmax.mat.check <- dat2.minmax.mat - mean(dat2.seq)
        for (i in seq_along(n.check.seq)){
          group.size <- n.check.seq[i]
          dat2.meanseq <- get.meanseq(dat2.seq, n.guess = group.size)
          dat2.minmax.mat.check[i,] <- c(min(dat2.meanseq), max(dat2.meanseq))
        }
      }
    }
    ...
    dat2.minmax.mat.sm <- dat2.minmax.mat.check
    dat2.minmax.mat.sm[,1] <- get.smooth.val(x=n.check.seq, y=dat2.minmax.mat.check[,1])
    dat2.minmax.mat.sm[,2] <- get.smooth.val(x=n.check.seq, y=dat2.minmax.mat.check[,2])
    
    #perform the test
    #stats.max.seq <- stats.min.seq <- numeric(length(n.check.seq))
    stats.min.seq <- dat2.minmax.mat.sm[,1]
    stats.max.seq <- dat2.minmax.mat.sm[,2]
    ...
    for (i in seq_along(n.check.seq)){
      ...
      #give the upper p-value for order statistics
      p.max.seq[i] <- uppvalue.maxmean(b, 
                                       sdl = sdl.est, sdr = sdr.est, m=m.check, 
                                       pf = pf.asym) #change m.check to N/n.check
      p.min.seq[i] <- uppvalue.minmean(a, 
                                       sdl = sdl.est, sdr = sdr.est, m=m.check, 
                                       pf = pf.asym)
      #compute the combined upper p-values
      # p.comb.seq[i] <- uppvalue.comb(a,b, sdl = sdl.est, sdr = sdr.est, m=m.check, 
      #                                pf = pf.asym)
      p.comb.seq[i] <- min(p.max.seq[i], p.min.seq[i])
    ...
    if (smooth.plot.test){
      ...
      #two threshold for V-values: c(0.1,0.05)
    } else {
      plot.test.obj <- ggplot(data = p.maxmin.dat, aes(x=groupsize.check, y=value))  + 
        #geom_point(aes(colour=variable))  + 
        geom_hline(yintercept = c(0.1,0.05), col = "brown", lty=2) + ggtitle("V-values") +
