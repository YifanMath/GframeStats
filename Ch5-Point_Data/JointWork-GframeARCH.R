...
noise.gen.eqblock <- function(N, gr.size, sd.set = c(0.5, 1, 2), 
                              z.ind = NULL){
  #z.ind should be consistent with sd.set
  m <- ceiling(N/gr.size)
  if (is.null(z.ind)){
    z.val <- sample(sd.set, m, replace = TRUE)
  } else {
    z.val <- sd.set[z.ind]
  }
  z.seq <- rep(z.val, each = gr.size)
  z.seq[seq_len(N)]*rnorm(N)
}
...
ar1.gen.sigdepn <- function(N, gr.size = 2e2, sdl=0.5, sdr=2, b.true=0.7, 
                            plot.ind = FALSE){
  #generate AR(1) from sig.seq that involves feedback
  ...
  for (i in seq_len(m-1)){
    ...
    for (t in ind.new2){
      ...
      x.seq[ind.new2[t+1]] <- x.seq[ind.new2[t]]*b.true + w.seq[ind.new2[t+1]]
    }
  }
  if(plot.ind){
    t.seq <- seq_len(N)
...
CI.cover.comp <- function(N=1e3, ar1.gen, name.char, 
                          save.plot = TRUE, 
                          print.plot = TRUE, 
                          rep.time=2e3, 
                          boot.time = 1e3,
