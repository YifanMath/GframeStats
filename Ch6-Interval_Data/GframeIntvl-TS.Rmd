...
- (The G-expectation framework does allow the sequence has classical dependence as long as )
...
plot.intvl(w.sq.cummean) #the variance uncertainty is [0,4]
```
...
plot.intvl(w12.seq1[seq_len(50),])
```
...
plot.intvl(w12.seq2[seq_len(50),])
```
...
```{r}
#also specify the width and height
pdf(file = "/Users/Yifan/Dropbox/00-Research/00-KeyFiles/04-G-frame-IntervalData/Figs/xobslineplot.pdf")
plot.intvl(x.obs[sub.ind,])
dev.off()
```
...
### Directly work on the observed intervals
...
### Use the distns of two ends of the observed intervals
...
lines(x.seq, dnorm.intvll(x.seq, sd.intvl = sd.intvl.true), type = "l", col=2)

#hist(x.obs[,2], probability = TRUE, ylim = ylim1, breaks = "scott")
hist(x.obs[,2], probability = TRUE, ylim = ylim1)
#lines(density(x.obs[,2]), col=2)
lines(x.seq, dnorm.intvlr(x.seq, sd.intvl = sd.intvl.true), type = "l", col=2)

#ecdf vs cdf
plot(ecdf(x.obs[,1]))
lines(x.seq, pnorm.intvll(x.seq, sd.intvl = sd.intvl.true), type = "l", col=2)

plot(ecdf(x.obs[,2]))
lines(x.seq, pnorm.intvlr(x.seq, sd.intvl = sd.intvl.true), type = "l", col=2)
```

### Comparison with G-normal cdf
...
ggplot(den.dat2, aes(x=x.seq, y=value, colour=variable)) + geom_line(aes(lty=variable))+scale_colour_manual(values=cbPalette)
```
...
### Use the distns of the center and range

```{r}
intvlnormal.ctrrng(x.obs)
#compare this method
#confidence interval 
```
...
```{r}
#compute the bias and SE
bias.function <- function(est, par.true=c(1,1,2)){
  bias <- est-par.true
  #se <- sd(est.seq)
  bias
}

#se.function <- function(est, par.true=c(1,1,2)){
#  (est-par.true)
#}
...
#bias.mat <- apply(ctrrng.est.mat, 2, bias.function)
#bias.mc <- apply(bias.mat, 1, mean)
#bias.mc
```
...
#it can retrieve the original dataset
```
...
```{r}
#comparison in different parameter setups and also sample size

```



### Ambiguity in Interval Direction
...
```{r}
#check the center-radius ratio
plot(x.cen/x.rad, type = "l")
hist(x.cen/x.rad)
```
...
library(Rfast)
#example("foldnormal")
normfold.est <- foldnorm.mle(x.rad)
est.par <- normfold.est$param
mu.rad <- est.par[1]
...
c(mu.low,mu.up,sig.low,sig.up)
```

```{r}
intvlnormal.ctrrng(x.obs, meanCertainty = FALSE)
```

```{r}
#check the sampling distribution
n <- 2e3
est.mat <- replicate(1e2, {
#sd.intvl.true <- c(-1,2)
#mean.intvl.true <- c(0,1)
w <- rsemiGnorm.intvl(n, sd.intvl = sd.intvl.true)
w.obs <- as.propintvl(w)
...
for (i in seq_len(nrow(est.mat))){
  hist(est.mat[i,], main = paste("Sampling Distribution of", main.seq[i]))
...
This simulation of $G$-BM is not in the traditional sense and it is also not for a fixed $\varphi$. 

```{r}
#
n <- 1e3
w.seq <- rsemiGnorm.intvl(n, sd.intvl = c(1,2))
w.seq.obs <- as.propintvl(w.seq)
...
# Illustration of semi-G-version CLT 
(also check existing reference on the limiting theorems regarding interval-data)

```{r}
n0 <- 1e3
x.intvl <- rsemiGdistn.intvl(n0, sd.intvl = c(1,2), 
...
```{r}
time.ind <- "2000/2020"
S.intvl.xts.close <- GSPC[time.ind, c("GSPC.Close")]
...
```{r}
return1 <- diff(S.intvl.xts.close)/S.intvl.xts.close[-1]
plot(return1*100)
lines(return.center.xts)

matplot(cbind(return1*100, return.center.xts), type = "l")
```
...
kurtosis(return.center.xts) #closing value 
...
Check the existing literature in interva-valued return data. 
...
### Possible Designs (simulation study)

I want to first point out that our goal in our current study is not about prediction but to study the behaviour of the possible underlying interval process. 

Here we just want to try a simulation study to mimic the behaviour of the interval-valued log return data.

- volatility model on a finer time grid; 
- consider the volatility model on the current time grid. 
...
```{r}
plot(return.intvl.rad, type = "l")
```

Can we say these methods are equivalent in some sense? 

```{r}
#alternatively
#we can also directly consider the time varying sigma interval 

```


#### The same time grid: constant sigma

```{r}
#
#return.intvl.num <- as.matrix(return.intvl.xts)
#rownames(return.center.num) <- NULL
#rng <- apply(return.intvl.num,1,range)
#return.intvl.sub <- return.intvl.num[1e3,]
#any(is.na(return.intvl.sub))
#intvlnormal.ctrrng(x.obs, meanCertainty = FALSE)
#x.sub <- intvl(left = as.numeric(return.intvl.sub[,1]), 
#               right = as.numeric(return.intvl.sub[,2]))

#
par.est <- intvlnormal.ctrrng(return.intvl.xts,
                              meanCertainty = FALSE)
par.est

N <- nrow(return.intvl.xts)
n <- floor(N/2)

par.est <- intvlnormal.ctrrng(return.intvl.xts[seq_len(n), ],
                              meanCertainty = FALSE)
par.est
...
#we can check that the mean parameter is almost certain
re <- rintvlnormal(2e3, par = c(0.05,0.05,par.est[c(3,4)]))
return.mod <- re$x.intvl.obs
ep <- re$ep
plot.intvl(return.mod)

#center-radius ratio

return.mod.cen <- apply(return.mod, 1, center)
return.mod.rad <- apply(return.mod, 1, radius)
plot(return.mod.cen/return.mod.rad, type = "l")
hist(return.mod.cen/return.mod.rad)

sdl <- par.est[3]
sdr <- par.est[4]
cenrad.ratio <- (sdl+sdr)/abs(sdr-sdl)*sign(ep)
#computation error 

#plot(cenrad.ratio, type = "l")
#hist(cenrad.ratio)
```

#### The same time grid: changing sigma
